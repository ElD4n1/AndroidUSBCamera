...
abstract class CameraView : ICameraStateCallBack {
    ...
    protected fun initView() {
        when (val cameraView = getCameraView()) {
            ...
            // offscreen render
            if (this == null) {
                throw NotImplementedError("offscreen render not supported")
            }
        }?.also { view->
    ...
    protected fun registerMultiCamera(context: Context) {
    ...
    protected fun openCamera(st: IAspectRatio? = null) {
        Logger.i(TAG, "openCamera(), view: $st")
        when (st) {
            ...
            Logger.i(TAG, "openCamera(), view: $this")
            getCurrentCamera()?.openCamera(this, getCameraRequest())
            getCurrentCamera()?.setCameraStateCallBack(this@CameraView)
        }
    }
